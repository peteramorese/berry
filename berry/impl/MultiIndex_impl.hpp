...
BRY::FixedNormIncrementer::FixedNormIncrementer(std::size_t sz, std::size_t index_constraint, bool begin, std::vector<std::size_t>& initial_idx)
...
std::size_t BRY::FixedNormIncrementer::indexConstraint() const {
    return m_l1_norm;
}

bool BRY::FixedNormIncrementer::increment(std::vector<std::size_t>& current_idx) {
...
bool BRY::FixedNormIncrementer::decrement(std::vector<std::size_t>& current_idx) {
...
void BRY::FixedNormIncrementer::updateIdx(std::vector<std::size_t>& current_idx) const {
    std::size_t d_i = 0;
    std::size_t sum = 0;
...
BRY::ExhaustiveIncrementer::ExhaustiveIncrementer(std::size_t sz, std::size_t index_constraint, bool begin, std::vector<std::size_t>& initial_idx)
    : m_linfty_norm(index_constraint)
{}

std::size_t BRY::ExhaustiveIncrementer::indexConstraint() const {
    return m_linfty_norm;
}


bool BRY::ExhaustiveIncrementer::increment(std::vector<std::size_t>& current_idx) {
    ++current_idx[0];
...
bool BRY::ExhaustiveIncrementer::decrement(std::vector<std::size_t>& current_idx) {
    --current_idx[0];
    for (std::size_t i = 0; i < current_idx.size() - 1; ++i) {
        if (current_idx[i] >= m_linfty_norm) {
            current_idx[i] = 0;
...
BRY::MultiIndex<INCREMENTER>::MultiIndex(std::size_t sz, std::size_t index_constraint, bool begin)
    : m_idx(sz, 0)
...
template <class INCREMENTER>
std::size_t BRY::MultiIndex<INCREMENTER>::indexConstraint() const {
...
template <class INCREMENTER>
std::size_t BRY::MultiIndex<INCREMENTER>::operator[](std::size_t d) const {
...
template <class INCREMENTER>
std::vector<std::size_t>::const_iterator BRY::MultiIndex<INCREMENTER>::begin() const {
    return m_idx.begin();
}

template <class INCREMENTER>
std::vector<std::size_t>::const_iterator BRY::MultiIndex<INCREMENTER>::end() const {
    return m_idx.end();
}
